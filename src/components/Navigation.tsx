...
const Navigation = () => {
  ...
  const NavigationButton = ({ path, Icon, label }: { path: string; Icon: any; label: string }) => (
    <button
      ...
      className={`flex flex-col items-center p-2 ${
        isActive(path) ? "text-[#638889]" : "text-[#9DBC98]"
      }`}
    >
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#F9EFDB] border-t border-[#EBD9B4]">
        ...
        <div className="flex justify-between py-2">
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            <SheetTrigger asChild>
              <button className="flex flex-col items-center p-2 text-[#9DBC98]">
